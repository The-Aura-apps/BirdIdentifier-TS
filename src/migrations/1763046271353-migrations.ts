...
export class Migrations1763046271353 implements MigrationInterface {
    name = 'Migrations1763046271353';

    public async up(queryRunner: QueryRunner): Promise<void> {
        // Check and create media_type_enum if it doesn't exist
        const mediaTypeExists = await queryRunner.query(`
        ...
        // Check and create conservation_statuses_code_enum if it doesn't exist
        const conservationStatusExists = await queryRunner.query(`
            SELECT 1 FROM pg_type WHERE typname = 'conservation_statuses_code_enum'
        `);

        if (conservationStatusExists.length === 0) {
            await queryRunner.query(
                `CREATE TYPE "public"."conservation_statuses_code_enum" AS ENUM('EX', 'EW', 'CR', 'EN', 'VU', 'NT', 'LC', 'DD', 'NE')`,
            );
        }

        // Check and create bird_distributions_season_enum if it doesn't exist
        const birdDistributionsSeasonExists = await queryRunner.query(`
            SELECT 1 FROM pg_type WHERE typname = 'bird_distributions_season_enum'
        `);

        if (birdDistributionsSeasonExists.length === 0) {
            await queryRunner.query(
                `CREATE TYPE "public"."bird_distributions_season_enum" AS ENUM('breeding', 'non-breeding', 'year-round', 'migration')`,
            );
        }

        // Check and create observations_status_enum if it doesn't exist
        const observationsStatusExists = await queryRunner.query(`
            SELECT 1 FROM pg_type WHERE typname = 'observations_status_enum'
        `);

        if (observationsStatusExists.length === 0) {
            await queryRunner.query(
                `CREATE TYPE "public"."observations_status_enum" AS ENUM('pending', 'processing', 'completed', 'failed')`,
            );
        }

        // REMOVED THE DUPLICATE CREATE TYPE STATEMENTS FROM HERE

        await queryRunner.query(
            `CREATE TABLE "media" ("id" SERIAL NOT NULL, "storageKey" character varying(500) NOT NULL, "type" "public"."media_type_enum" NOT NULL DEFAULT 'photo', "size" character varying, "caption" character varying(255), "source" character varying(255), "attribution" character varying(255), "orderIndex" integer NOT NULL DEFAULT '0', "metadata" jsonb, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "bird_id" integer NOT NULL, CONSTRAINT "PK_f4e0fcac36e050de337b670d8bd" PRIMARY KEY ("id"))`,
        );
        await queryRunner.query(
        ...
        await queryRunner.query(
            `CREATE TABLE "bird_foods" ("id" SERIAL NOT NULL, "isActive" boolean NOT NULL DEFAULT true, "created_at" TIMESTAMP NOT NULL DEFAULT now(), "updated_at" TIMESTAMP NOT NULL DEFAULT now(), "bird_id" integer NOT NULL, "food_id" integer NOT NULL, CONSTRAINT "PK_132e19a7434923cbab9ca6c3234" PRIMARY KEY ("id"))`,
        );
        await queryRunner.query(
            `CREATE UNIQUE INDEX "IDX_7c8d76a4530e449b670ad3fda0" ON "bird_foods" ("bird_id", "food_id") `,
        );
        await queryRunner.query(
            `CREATE TABLE "common_names" ("id" SERIAL NOT NULL, "name" character varying NOT NULL, "language" character varying(10) NOT NULL DEFAULT 'en', "region" character varying NOT NULL, "createdAt" TIMESTAMP NOT NULL DEFAULT now(), "updatedAt" TIMESTAMP NOT NULL DEFAULT now(), "bird_id" integer NOT NULL, CONSTRAINT "PK_47a1748c20d65a3b5aa528aa858" PRIMARY KEY ("id"))`,
        );
        await queryRunner.query(
            `CREATE UNIQUE INDEX "IDX_ac0ee6ac532a46891fca986220" ON "common_names" ("bird_id", "name", "language") `,
        );
        // REMOVED: CREATE TYPE "public"."conservation_statuses_code_enum"...
        await queryRunner.query(
        ...
        await queryRunner.query(
            `CREATE TABLE "habitats" ("id" SERIAL NOT NULL, "name" character varying(100) NOT NULL, "description" text, CONSTRAINT "UQ_63cfbf4a9393c728c9673440298" UNIQUE ("name"), CONSTRAINT "PK_1d6e7a888814106d17f07d7d418" PRIMARY KEY ("id"))`,
        );
        await queryRunner.query(
        ...
        // REMOVED: CREATE TYPE "public"."observations_status_enum"...
        await queryRunner.query(
        ...
        await queryRunner.query(
            `ALTER TABLE "birds" ADD CONSTRAINT "FK_d93529f0ca0d61032ba30f5d1f3" FOREIGN KEY ("taxonomy_id") REFERENCES "taxonomy"("id") ON DELETE SET NULL ON UPDATE NO ACTION`,
        );
        await queryRunner.query(
            `ALTER TABLE "birds" ADD CONSTRAINT "FK_e35a144a6805de1313daf95e027" FOREIGN KEY ("conservation_status_id") REFERENCES "conservation_statuses"("id") ON DELETE SET NULL ON UPDATE NO ACTION`,
        );
        await queryRunner.query(
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        // Keep the down method as is for now
        await queryRunner.query(
