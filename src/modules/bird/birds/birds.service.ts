...
export class BirdsService {
  ...
  async create(createBirdDto: CreateBirdDto): Promise<Bird> {
    const {
      ...
      habitatIds,
      media: mdiDto,
      distrebutions: distributionsDto,
      birdFood: birdFoodsDto,
      ...rest
    } = createBirdDto;
    ...
    // Handel Media
    // let media: Media[] = [];
    // if (mdiDto && mdiDto.length > 0) {
    //   media = mdiDto.map((dto) =>
    ...
    const bird = this.birdRepo.create({
      ...
      birdFoods, // Array of junction entities (ManyToMany with junction)
      ...rest,
    });
